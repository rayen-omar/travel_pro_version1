...
class TravelReservation(models.Model):
    ...
    local_or_foreign = fields.Selection([
        ('local', 'Hôtel local'),
        ('foreign', 'Hôtel étranger')
    ], string='Local ou étranger', required=True, default='local')

    room_category = fields.Selection([
        ('standard', 'Chambre Standard'),
        ('ls', 'Logement simple (LS)'),
        ('autre', 'Autre')
    ], string='Chambre N°1', required=True)
    ...
    # === ENREGISTRER LA RÉSERVATION (OBLIGATOIRE) ===
